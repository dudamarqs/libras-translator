...
from libras.vision.hands import CONEXOES, N_LANDMARKS, NOMES_LANDMARKS, DetectorMaos
...
class TestConverter:
    def test_sem_maos_produz_resultado_vazio(self) -> None:
        r = DetectorMaos._converter(FakeResultado([], [], []))
        assert r.vazio
        assert r.maos == ()
    ...
    def test_uma_mao_vira_arrays_numpy_com_shape_correto(self) -> None:
        ...
        r = DetectorMaos._converter(bruto)
    ...
    def test_nao_confunde_landmarks_normalizados_com_world(self) -> None:
        ...
        (mao,) = DetectorMaos._converter(bruto).maos
